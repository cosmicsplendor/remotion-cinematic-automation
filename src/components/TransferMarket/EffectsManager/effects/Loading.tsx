...
const LoadingEffect: React.FC<{
    effect: LoadingEffect & { reverse?: boolean }; // Extended to include reverse flag
    ...
    useEffect(() => {
        ...
        // Create the trail line (appears behind the star) - only if not reverse
        if (!effect.reverse) {
    ...
    }, [svgRef.current, groupId, effect.reverse]); // Re-run if SVG ref, group ID, or reverse flag changes
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            if (!groupElRef.current || !targetEl || frame0 === null || !loadingRectRef.current || !starPathRef.current) {
                return;
            }
            ...
            let progress = easingFns.sineInOut(t / effect.duration);
            
            // If reverse is true, invert the progress
            if (effect.reverse) {
                progress = 1 - progress;
            }
            ...
            const fadeOutStart = effect.duration - 0.75;
            const fadeOutDuration = effect.duration - fadeOutStart;

            if (t <= fadeInDuration) {
                // Fade in phase - use sineIn easing
                const fadeInProgress = t / fadeInDuration;
                currentOpacity = easingFns.sineIn(fadeInProgress);
            } else if (t >= fadeOutStart) {
                // Fade out phase - use sineOut easing
                const fadeOutProgress = (t - fadeOutStart) / fadeOutDuration;
                currentOpacity = 1 - easingFns.sineOut(fadeOutProgress);
            }
            // Middle phase: full opacity (currentOpacity remains 1)
            ...
            // For reverse animation, keep the bar positioned from the left but shrink it
            let barX = targetBox.x - POSITION_PADDING;
            let barWidth = currentWidth + POSITION_PADDING * 2;
            
            // No change in positioning for reverse - the bar always starts from left
            // The reverse effect comes from the progress calculation above

            // Apply position padding for better alignment during movement
            loadingRectRef.current.setAttribute('x', barX.toString());
            loadingRectRef.current.setAttribute('y', (targetBox.y - POSITION_PADDING).toString());
            loadingRectRef.current.setAttribute('width', barWidth.toString());
            ...
            let starX = targetBox.x + currentWidth;
            // No change needed for reverse - star follows the currentWidth which is already inverted
            ...
            // --- Animate Trail Line (only if not reverse and trail line exists) ---
            if (!effect.reverse && trailLineRef.current) {
                const trailLength = Math.min(currentWidth * 0.4, 100); // Longer trail (40% of progress or max 100px)
    ...
    }, [targetEl, frame, frame0, fps, effect.duration, effect.reverse, removeEffect]); // Dependencies for useEffect
